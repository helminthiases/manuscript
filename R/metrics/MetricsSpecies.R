...
MetricsSpecies <- function (pathstr) {
  ...
  daly <- read.csv(file = file.path(pathstr, 'data', 'daly_level_4.csv')) %>%
  ...
  prevalence <- read.csv(file = file.path(pathstr, 'data', 'prev_level_4.csv')) %>%
    dplyr::filter(disease %in% diseases)
