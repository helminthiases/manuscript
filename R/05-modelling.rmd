...
... summarises this chapter's common notations
...
The **preliminary model** algorithm in focus is the **generalised linear mixed model** (GLMM) of the form:
\begin{align}
...
It is a Binomial GLMM because the outcome values, prevalence, are proportions.  \cref{tab:T0004} defines the terms of each equation.  Additionally, $b_{i}$ & $c_{i}$ are the random effects terms in relation to ``identifier`` & ``year``, respectively.  \cref{eq:0002} outlines the assumptions underlying the random effects vectors, i.e., each is
...
The expression $\mathbf{\delta}(x_{i})^{T} \mathbf{\beta}$ (\cref{eq:0001}) is the *fixed effects* expression; $\mathbf{\delta}(x_{i})$ is a vector of terms that depend on one or more independent variables, whilst $\mathbf{\beta}$ is the vector of associated regression coefficients (\cref{tab:T0001}).  In this investigation, the independent variables are the aforementioned WaSH variables, population density, and elevation (\cref{tab:T0003}).
...
Let $0.05$ be the $p_{value}$ significance level, then there is evidence in favour of the alternative hypothesis, $H_{1}$, if the estimated coefficient's $p_{value} < 0.05$.  The comparison of the models is via deviance/AIC (Akaike Information Criterion).  The models are

\begin{equation}
  \mathbf{\delta}(x_{i}) =
  \begin{cases}
    [1, piped\underline{\hspace{0.125cm}}sewer, ] \\
    [1, piped\underline{\hspace{0.125cm}}sewer, ]
  \end{cases} (\#eq:0005)
\end{equation}
