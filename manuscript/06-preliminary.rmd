...
& \quad  + \beta_{3}{elevation.km}(x_{i}) + b_{i} + c_{i} \nonumber
\end{align}
Each reflects the sometimes inconclusive relationships, between *prevalence* & *extraneous independent variables*, outlined within the \hyperref[chapter:exploration]{exploratory analysis} chapter (\cref{fig:F0003,F0013,F007}).
...
Leading on from the aforementioned preliminary models, with linear predictors \cref{eq:0029,0030}, this section focuses on models with linear predictors
...
and
\begin{align}
\eta_{i} & = \beta_{0} + \beta_{1}{piped\underline{\hspace{0.125cm}}sewer}(x_{i}) + \beta_{2}{ln(p\underline{\hspace{0.125cm}}density)}(x_{i}) (\#eq:0032) \\
& \quad  + \beta_{3}{elevation.km}(x_{i}) + b_{i} \nonumber
\end{align}
