...
\cref{tab:T0025} & \cref{tab:T0026} outline the variance, and scale parameter, estimates of linear predictors \cref{eq:0040} & \cref{eq:0041}, respectively.  The estimates are different.  In general, \cref{tab:T0026} has narrower confidence interval widths; however the stochastic process $S(\mathbf{x})$ variances,  $\sigma^{2}$, of \cref{tab:T0025} & \cref{tab:T0026} have similar interval widths.
...
logarithmic$special %>%
  ...
  kableExtra::kable(
    ...
    linesep = '\\addlinespace',
    col.names = kableExtra::linebreak(c('', 'est.', 'lower', 'upper', '$e^{est.}$', 'lower', 'upper')),
    caption = 'The Binomial geostatistical logistic model variance estimates vis-\\`{a}-vis $\\beta_{0} + \\beta_{1}{piped\\underline{\\hspace{0.125cm}}sewer}(x_{i}) + \\beta_{2}{ln(p\\underline{\\hspace{0.125cm}}density.k)}(x_{i})  + \\beta_{3}{elevation.km}(x_{i})  + S(x_{i}) + U_{i}$.  $\\kappa = 0.5$') %>%
  kableExtra::kable_styling(position = 'center', latex_options = 'hold_position', font_size = 8, full_width = FALSE) %>%
  kableExtra::add_header_above(c('', '', 'confidence\ninterval' = 2, '', 'confidence\ninterval' = 2)) %>%
  kableExtra::add_footnote(label = 'est.: estimate', notation = 'none')  %>%
  kableExtra::column_spec(column = 1, latex_column_spec = "c")
...
Modelling the ... is a core function of Binomial geostatistical logistic models.  Empirical variograms assess via the standardises residual errors of a model.
...
The empirical variogram w.r.t. each model's standardised residual errors 
